...
const getBackgroundColor = (is_day: number) => {
    return is_day === 1 
        ? "linear-gradient(to bottom right, #f6d365, #ffffff)" // Orange to white gradient for daytime
        : "linear-gradient(to bottom right, #b5c1ff, #ffffff)"; // Blue to white gradient for nighttime
};

const TemperatureBound: React.FC<TemperatureBoundProps> = ({ is_day, temp_max, temp_min }) => {
